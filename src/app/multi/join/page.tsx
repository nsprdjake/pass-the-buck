...
function JoinForm() {
  ...
  const [code, setCode] = useState(
    (search.get("code") ?? "").toUpperCase().replace(/[^A-Z0-9]/g, "")
  );
  ...
  return (
    <form onSubmit={handleJoin} className="space-y-4">
      ...
      {!user && (
        <p
          className="text-center text-[0.72rem] italic text-[var(--parchment-light)]/55"
          style={FELL}
        >
          Playing as a guest. Sign in to earn eyeBucks and track wins.
        </p>
      )}
    </form>
  );
